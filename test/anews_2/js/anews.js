...
async function loadFolders() {
	...
	mainContainer.innerHTML = '<div class="loading-text">Загрузка новостей...</div>';
...
function renderMainList(data) {
    ...
    data.forEach(item => {
        ...
        card.innerHTML = `
            <div class="card-media-box" id="${mediaContainerId}"></div>
            <div class="card-content">
                <b class="card-title" onclick="openAnimeNews('${item.id}', '${item.name}')">${item.name}</b>
            </div>
        `;
...
function loadMediaPreview(id, containerId) {
    ...
    playerWrapper.className = 'custom-video-wrapper';
    ...
    const svgPlay = `<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor" style="margin-left: 2px;"><path d="M8 5v14l11-7z"/></svg>`;
    const svgPause = `<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>`;

    const customControls = document.createElement('div');
    customControls.className = 'player-controls-overlay';
    customControls.innerHTML = `
        <button class="player-play-btn">${svgPlay}</button>
    ...
    playerWrapper.appendChild(customControls);

    const playBtn = customControls.querySelector('.player-play-btn');
    const progressBar = customControls.querySelector('.player-timeline-progress');
...
async function openAnimeNews(id, animeName) {
	...
	newsContainer.innerHTML = '<div class="loading-text">Загрузка новостей тайтла...</div>';
    toggleScreens(true);
...
async function loadSubtitleComment() {
    const subCommentEl = document.getElementById('site-subtitle-comment');
    if (!subCommentEl) return;

    try {
        const res = await fetch('https://raw.githubusercontent.com/REDYQ/Anime_NEWS/refs/heads/main/file/comment', { cache: 'no-store' });
        if (!res.ok) throw new Error();
        const text = await res.text();
        
        if (text && text.trim() !== '') {
            subCommentEl.textContent = text.trim();
            subCommentEl.style.display = 'block';
        } else {
            subCommentEl.style.display = 'none';
        }
    } catch (e) {
        subCommentEl.style.display = 'none';
    }
...
document.addEventListener('DOMContentLoaded', loadSubtitleComment);