...
const JSON_MAIN = 'https://raw.githubusercontent.com/REDYQ/Anime_List/refs/heads/main/file/data_test.json'; // ▶️ AList data
...
async function renderList(data, containerId, isMyList = false, userItems = []) {
    ...
    for (const item of data) {
        ...
        const myMatchingTitles = subData ? subData.filter((sub, index) => {
            const currentId = `${item.id}_${index + 1}`;
            const userItem = userItems.find(u => u.id === currentId);
            return isMyList ? (userItem && filterLogic(item, sub, userItem, true)) : filterLogic(item, sub, null, false);
        }).length : -1;
