...
async function renderList(data, containerId, isMyList = false, userItems = []) {
	...
	for (const item of data) {
		...
		if (isSearchActive && myMatchingTitles === 0 && !currentFilters.category.includes('can_finish')) continue;
...
async function fetchSubItems(folderId, container, userItems = [], isMyList = false) {
	try {
		...
		container.innerHTML = subData.map((sub, index) => {
			...
			if (isMyList && myListMode === 'titles' && !isSelected && !currentFilters.category.includes('can_finish')) return '';
			...
			const dimmedStyle = (isMyList && myListMode === 'folders' && !isSelected && !currentFilters.category.includes('can_finish')) 
